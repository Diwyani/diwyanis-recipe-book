...
const fetchRecipes = unstable_cache(
  async (): Promise<Recipe[]> => {
    ...
    const { data, error } = await supabase
      .from("recipes")
      .select("id, title, slug, image_url, category, time_minutes, cost_inr, ingredients, instructions")
      ...
      <div className="mx-auto max-w-6xl">
        <header className="grid gap-12 md:grid-cols-[minmax(0,1fr)_auto] md:items-start md:gap-16 lg:gap-24">
          <div className="max-w-xl space-y-3">
            <p className="text-[24px] font-medium tracking-wide">Diwyani&apos;s</p>
            ...
            <div className="max-w-md text-base leading-8 md:text-lg md:leading-9 space-y-5">
              ...
              <p>The only real rule here is keep it light on money, light on time, and just healthy enough but colourful enough that I actually want to eat it.</p>
            </div>
          </div>
